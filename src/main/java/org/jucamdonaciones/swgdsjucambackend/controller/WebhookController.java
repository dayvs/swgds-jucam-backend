...
public class WebhookController {
    ...
    public ResponseEntity<?> recibirWebhook(@RequestBody WebhookPayload payload) {
        ...
        // Solo proceder si el status es CHECKOUT_COMPLETED o CHECKOUT_PENDING
        if (!("CHECKOUT_COMPLETED".equals(status) || "CHECKOUT_PENDING".equals(status))) {
            System.out.println("Status no permite proceder: " + status);
            return ResponseEntity.ok("Webhook recibido, pero no se continúa el flujo (status: " + status + ")");
        }

        // Extraer receipt_no para usarlo en el siguiente paso
        String receiptNo = (String) checkoutResponse.get("receipt_no");
        if (receiptNo == null) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("No se encontró receipt_no en la respuesta");
        }
        ...
        // Aquí podrías combinar la información obtenida de checkoutResponse y paymentDetails
        // y proceder a guardarla en la base de datos según sea necesario.
